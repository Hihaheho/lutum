//! Ollama transcript example - OpenAI / Responses backend
//!
//! Requires a running Ollama instance with the qwen3.5 model:
//!   ollama pull qwen3.5
//!   cargo run --example ollama_transcript -p agents-openai

use std::io::Write;
use std::sync::Arc;

use agents_openai::OpenAiAdapter;
use agents_protocol::{
    budget::Usage,
...
async fn main() -> anyhow::Result<()> {
    ...
    let adapter = OpenAiAdapter::new("ollama").with_base_url(BASE_URL);
    ...
    println!(
        "{}  model={}  seed={}  base_url={}",
        style("OpenAI / Responses API").bold(),
        style(MODEL).cyan(),
        style(SEED).yellow(),
        style(BASE_URL).dim(),
    );
